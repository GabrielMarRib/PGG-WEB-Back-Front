...
const NavBar = () => {
  ...
  <BsArrowLeftShort
    className={`bg-white text-black text-3xl rounded-full absolute -right-3 top-9 border border-dark cursor-pointer ${!open && "rotate-180"}`}
  ...
  <ul className="pt-2 h-full overflow-y-auto submenu">
    {Menus.map((menu, index) => (
      ...
      {/* Content Area */}
      <div className="p-7">
        <a id="NomeEmpresa" onClick={() => { navigate("/") }}>- Conteudo Site - </a>
      </div>
    </div>
  );
};
