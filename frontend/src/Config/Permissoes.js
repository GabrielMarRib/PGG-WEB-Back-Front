...
export const pegaPermissoesWHERE = async (where, context) => {
  try {
    ...
    if (context)
      return {
        data: response.data[0].Permissoes,
        nome: response.data[0].nome_grupo,
      };
...
export const checaPermissaoVisualizacao = (permissao, role, intent) => {
  ...
  const check =
    permissaoParse[role].permissoes[intent.class][intent.intentPage]
      .visualizacao;
  return check;
};
...
export const checaPaiPermissaoEspecifico = (
  permissao,
  role,
  classJSON,
  chave
